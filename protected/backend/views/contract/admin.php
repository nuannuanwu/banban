                ...
                <?php if(count($data['model'])){  ?>
                    <?php foreach($data['model'] as $con){?>
                        <tr>
                            ...
                            <td><?php echo $con->getBusinessName();?></td>
                            <td><?php echo $con->getStateName(); ?></td>
                            <td><?php echo date('Y-m-d H:i',strtotime($con->creationtime));?></td>
                            <?php if($con->state==0 || $con->state==3){ ?>
                                <td><a href="<?php echo Yii::app()->createUrl('contract/update/'.$con->cid);?>">编辑</a></td>
                            <?php }else{ ?>
                                <td><a href="<?php echo Yii::app()->createUrl('contract/updateview/'.$con->cid);?>">查看</a></td>
                            <?php } ?>
                        </tr>
                     <?php } ?>
                <?php }else {?>
