                ...
                <?php if(count($data['model'])){  ?>
                    <?php foreach($data['model'] as $foc){?>
                        <tr>
                            ...
                            <td><?php echo $foc->getBusinessName();?></td>
                            <td><?php if($foc->countConFocRelation()){echo "已配置";}else{echo "未配置";} ?></td>
                            <td><?php echo date('Y-m-d H:i',strtotime($foc->creationtime));?></td>
                            <?php if($foc->countConFocRelation()){?>
                                <td><a href="<?php echo Yii::app()->createUrl('focus/updateview/'.$foc->fid);?>">查看</a></td>
                            <?php }else{ ?>
                                <td><a href="<?php echo Yii::app()->createUrl('focus/update/'.$foc->fid);?>">编辑</a></td>
                            <?php } ?>
                        </tr>
                     <?php } ?>
                <?php }else {?>
