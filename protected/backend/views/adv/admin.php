                ...
                <?php if(count($data['model'])){  ?>
                    <?php foreach($data['model'] as $adv){?>
                        <tr>
                            ...
                            <td><?php echo Business::getBusinessName($adv->bid);?></td>
                            <td><?php if($adv->countConAdvRelation()){echo "已配置";}else{echo "未配置";} ?></td>
                            <td><?php echo date('Y-m-d H:i',strtotime($adv->creationtime));?></td>
                            <?php if($adv->countConAdvRelation()){?>
                                <td><a href="<?php echo Yii::app()->createUrl('adv/detail/'.$adv->aid);?>">查看</a></td>
                            <?php }else{ ?>
                                <td><a href="<?php echo Yii::app()->createUrl('adv/update/'.$adv->aid);?>">编辑</a></td>
                            <?php } ?>
                        </tr>
                    <?php } ?>
                <?php }else {?>
