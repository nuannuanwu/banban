...
<div class="box">
    <div class="form tableBox">
        <form id="business-form" method="post" action="<?php echo Yii::app()->createUrl("class/saveexcel");?>">
                               ...
                               <tbody>
                                   <tr>
                                       <td>
                                           ...
                                           <span id="Validform_file"  class="Validform_checktip ">&nbsp;&nbsp;<a  href="<?php echo Yii::app()->request->baseUrl.'/template/班级导入模板.xls'; ?>">下载格式模版 </a></span>
...
function ajaxFileUpload(){
    ...
    var ajaxurlUpload = '<?php echo Yii::app()->createUrl('/class/import');?>'+'?schoolid='+schoolid;
    $('#result').empty();
    $.ajaxFileUpload({
            ...
            if(arr.status=='1'){ //成功
                var errNum= $.trim(arr.errNum);
                 ...
                 var shtml='<a id="fileName" class="fileIoc" href="javascript:void(0);">'+arr.filename+' </a>  一共：'+
                     (arr.validnum+arr.novalidnum)+' 条数据，'+arr.validnum+' 条符合条件，<span class="remind" style="color: red;">'
                     +arr.novalidnum+'条无效数据（其中 ' +arr.notgradenum+' 条数据的年级不存，'+arr.notclassnum+'条数据的班级名称已存在)'+(errNum!='' ?('，文件错误行号：'+errNum):'')+'</span>';
                $('#result').html(shtml);
                $("#fileChecktip").val(1);
                $('#upfileloading').hide();
