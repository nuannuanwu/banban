...
<div id="mainBox" class="mainBox">  
    <div id="contentBox" class="cententBox"> 
        <div class="titleHeader bBttomT">
            <span class="icon icon3"></span>我的班班 > 班费转出
        </div>

        <div class="box" style="padding:15px 25px;">
            <nav class="navMod">
                <a href="<?php echo Yii::app()->createUrl('expense/expdetail/' . $cid, array('authority'=>$authority)); ?>" class="btn btn-default"><img class="return" src="<?php echo Yii::app()->request->baseUrl; ?>/image/banban/get_back_ico.png" alt="">返回</a>
            </nav>
            <!--<div class="payment">
               <div class="pay-type">
            ...
            <div class="pay-form" >
                <form class="transfer-form" action="" method="post" id="formBoxRegister" name="" onkeydown="if (event.keyCode == 13) {return false;}">
                    <input type="hidden" name="Tenpay[cid]" value="<?php echo $cid; ?>"/>
                        ...
                        <tbody>
                            ...
                            <tr class="tr-bb">
                                <td>
                                    ...
                                    <input type="hidden" name="totalFeeHidden" id="totalFeeHidden" value="<?php echo isset($totalFee) ? $totalFee : 0; ?>" />
                                    <div class="inputBox"><input id="pay_amt"  name="Tenpay[pay_amt]" maxlength="7" class="mediumM" type="text" value="<?php echo isset($tenpay['pay_amt']) ? $tenpay['pay_amt'] : ''; ?>" datatype="num3" placeholder="￥0.0"></div>
                                    <span class="ValidformTip Validform_checktip"></span>
                                </td>
                            </tr>
                            <tr class="tr-bb">
                            ...
                            <tr>
                                <td style=" position: relative;"> 
                                    ...
                                    <p  style=" margin-top: 10px; margin-left: 200px; color: red;">
                                        提示：班费转出申请提交之后，班班会在第一时间处理您的申请，<br/>并于3日内用手机短信的方式将结果反馈给您。</p>
                                </td> 
                            </tr>
                        </tbody>
                    </table>  
...
<script>
        ...
        $(function () {
            ...
            $('#paySubmit').click(function () {
                ...
                var checkexpenseurl = "<?php echo Yii::app()->createUrl('ajax/checkexpense'); ?>";
                ...
                if (mobile != "") {
                    ...
                    if (eg.test(mobile) && mobile.length == 11) {
                        if (verifyCode != "") {
                            ...
                            var codeS = ajaxPost(checkurl, '4', mobile, verifyCode);
                            //if (codeS == '1') {
                                off = false;
                                countdown = 60;
                                ...
                                $.post(checkexpenseurl, {'cid': cid, 'amt': amt, 'uname': uname, 'ty': ty}, function (data) {
                                    if (data.status != 1) {
                                        //$("#transRst").val('2');	//这个值控制弹出窗口按钮的后续动作
                                        $("#boxtitle").text('转出失败');
                                        $("#isLeavePopupBox .remindInfo").html(data.msg);
                ...
                if (parseInt(countdown) == 60) {
                    ...
                    if (mobile == "") {
                        ...
                        $('#verifyCodeTip').text('').hide();
                            var code = ajaxPost(sendurl, '4', mobile, '');
                            if (code == '1') {
                                settime($(this));
                            }else if(code=="2"){
...
// 	    			  t.addClass("Validform_wrong").removeClass("Validform_right").text("班费余额不足！");
// 	    		  }
// 	    	}
            ...
            if (transRst && transRst != '') {
                ...
                if (parseInt(transRst) == 1) {
                    $("#boxtitle").text('提交成功');
                    $("#isLeavePopupBox .remindInfo").html('提交成功！共转出' +<?php echo isset($tenpay['pay_amt']) ? $tenpay['pay_amt'] : 0; ?> + '元，当前班费余额' +<?php echo isset($totalFee) ? $totalFee : 0; ?> + '元，预计三个工作日内到账，届时会有短信通知。');
                    showPromptPush('#isLeavePopupBox');
                } else if (parseInt(transRst) == 2) {
                    $("#boxtitle").text('提交失败');
                    $("#isLeavePopupBox .remindInfo").html('班费提交失败，请重新填写。'+errorMsg);
                    showPromptPush('#isLeavePopupBox');
                    t.addClass("Validform_wrong").removeClass("Validform_right");
                    //.text("班费提交失败，请重新填写。")
                } else if (parseInt(transRst) == 3) {
                    $("#boxtitle").text('提交失败');
                    $("#isLeavePopupBox .remindInfo").html("班费余额不足" + <?php echo Constant::EXPENSE_FIRST_TRANSFER; ?> + "元，无法转出。");
                    showPromptPush('#isLeavePopupBox');
                    t.addClass("Validform_wrong").removeClass("Validform_right").text("班费余额不足" + <?php echo Constant::EXPENSE_FIRST_TRANSFER; ?> + "元，无法转出。");
                } else if (parseInt(transRst) == 5) {
                    $("#boxtitle").text('提交失败');
                    $("#isLeavePopupBox .remindInfo").html("班费余额不足" + <?php echo Constant::EXPENSE_FIRST_TRANSFER; ?> + "元，无法转出。");
                    showPromptPush('#isLeavePopupBox');
                    t.addClass("Validform_wrong").removeClass("Validform_right").text("班费余额不足" + <?php echo Constant::EXPENSE_FIRST_TRANSFER; ?> + "元，无法转出。");
                } else if (parseInt(transRst) == 6) {
                    $("#boxtitle").text('提交失败');
                    $("#isLeavePopupBox .remindInfo").html('超出您本月转出所剩限额。');
                    showPromptPush('#isLeavePopupBox');
                    t.addClass("Validform_wrong").removeClass("Validform_right").text("超出您本月转出所剩限额。");
                } else if (parseInt(transRst) == 4) {
                    $("#boxtitle").text('提交失败');
                    $("#isLeavePopupBox .remindInfo").html('班费余额不足。');
                    showPromptPush('#isLeavePopupBox');
                    t.addClass("Validform_wrong").removeClass("Validform_right").text("班费余额不足。");
                }
            }
            ...
            $("#boxconfirm").click(function () {
                ...
                if (parseInt(transRst) == 1 || parseInt(transRst) == 2) {
                    location.href = "<?php echo Yii::app()->createUrl('expense/expdetail/' . $cid, array('authority'=>$authority)); ?>";
...
<script>
    //判断手机号码是否一致
    var shareUrl = '<?php echo Yii::app()->createAbsoluteUrl('mobile/cost_preview', array('classid'=>$cid,'uid'=>Yii::app()->user->id));?>';
    window._bd_share_config = {
        common: {
            bdText: '晒班费啦~我们班已经赚了<?php echo $totalFee; ?>元啦！',
            bdDesc: '动动小手，班费到手。把你们班费晒出来看看吧~',
            bdUrl: shareUrl,
...
//                }else if(cmd=='tsina'){ 
//                }
               // return {bdDesc: '我在班班支取了' + $("#pay_amt").val() + '元班费。发通知、发作业，手机轻松玩转家校沟通，动动手指还能挣班费！我们班已经挣了元了，你也快来一起为班级挣福利吧！'};
            },
