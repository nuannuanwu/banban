    ...
    <div id="contentBox" class="cententBox">
        ...
        <div class="box">
            <nav class="navMod navModDone" >
                <a href="<?php echo Yii::app()->createUrl('class/'.($type==2?'students/':'teachers/').$class->cid); ?>" class="btn btn-default"><img class="return" src="<?php echo Yii::app()->request->baseUrl; ?>/image/banban/get_back_ico.png" alt="">返回</a>
            </nav>
            <div class="formBox">
                ...
                <?php endif;?>
                <?php if(is_array($activelist) &&!empty($activelist)):?>
                <h3 class="uninvite-head">以下用户已激活，不发送短信（<?php echo count($activelist);?>）！</h3>
                <table class="uninvite-table">
                    <?php foreach($activelist as $val):?>
                    <tr>
                ...
                <?php endif;?>
                <?php if(is_array($sendthreelist) &&!empty($sendthreelist)):?>
                <h3 class="uninvite-head">以下用户已接受3次邀请而还未注册，不发送短信（<?php echo count($sendthreelist);?>）！</h3>
                <table class="uninvite-table">
                    <?php foreach($sendthreelist as $val):?>
                    <tr>
                        <td class="name"><?php echo $val['name'];?></td>
                        <td class="phone"><?php echo $val['mobile'];?></td>
                    </tr>
                ...
                <div class="invite-action">
                    <a class="btn btn-orange" href="<?php echo Yii::app()->createUrl('class/'.($type==2?'students/':'teachers/').$class->cid); ?>">返回“<?php echo $class->name;?>”</a>
                </div>
            </div>
        </div>
    </div>
