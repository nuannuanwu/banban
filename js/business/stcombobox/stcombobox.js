...
var STComboBox = (function($) {
    ...
    Class.prototype.Init = function(containerId,obj) { 
        ...
        this.getInput().on('keydown', function(evt) {
            ...
            if(self.getInput().val()==''){
                //self.getInput().val($("#statesComboInput").attr('sname'));
            }else{
                //self.getInput().attr({sname:'',sid:''});
                //$("#statesComboInput").val('').attr({sid:'',sname:''});
            }
    ...
    Class.prototype.filterAndResetSelected = function() {
        ...
        var sname = $('#statesComboInput').attr('sname'), sid = $('#statesComboInput').attr('sid'); 
        this.$('ddi').attr({'sname':sname,'sid':sid});
    };
    ...
    //处理程序keydown事件时触发在组合框
    Class.prototype.onKeyDown = function(evt,obj) {
        if(evt.which == keycodes.enter) {
            var rows = this.getVisibleDomRows();
            var selectedIndex = $(rows[this.selectedIndex]).attr('data-stc-id')*1;
            this.$('ddi').val(this.data[selectedIndex].texts);
            this.$('ddi').attr({sname:this.data[selectedIndex].texts,sid:this.data[selectedIndex].vid});
            this.hideList();
            if(this.onSelect) {
                this.onSelect(evt, this.getInput().val()); 
            }
            var sid = this.data[selectedIndex].vid;  
            //$('#statesComboInput').val(sid).attr('sname',this.data[selectedIndex].texts);
            ajaxFunction(sid,obj)//自加联动请求函数
            return false;
        } 
        ...
        if(evt.which == keycodes.clear){
            $("#statesComboInput").val(''); 
        }
    ...
    Class.prototype.selectRow = function(index, context) {
        ...
        switch(context) {
            case 0:
                $listContainer.scrollTop(rowPos.top); //just move to the item //选着移动
                break;
            case 1:
                if(rowPos.top < scrollTop) { //the user is moving up 向上选着
                    $listContainer.scrollTop(rowPos.top);
                }
                break;
            case 2:
                if((rowPos.top + rowHeight) > scrollBottom) { //the user is moving down 向下选着
    ...
    Class.prototype.populateList = function(datas,obj) { 
        var data =datas; 
        this.data = data;
        this.filterCache = '';
        for(var i=0; i < data.length; i++) {
            this.filterCache += i + '`' + data[i].texts + '#';
        }
        ...
        $list.find('.stc-lrow').hover(function() {
            ...
            self.getInput().val(self.data[selectedIndex].texts);
            self.getInput().attr("sname",self.data[selectedIndex].texts); 
            self.getInput().attr("sid",self.data[selectedIndex].vid);
            if(self.onSelect) {
                self.onSelect(evt, self.getInput().val());
            } 
            var sid = $(this).attr('data-stc-sid');
            $('#statesComboInput').val(sid).attr('sname',self.data[selectedIndex].texts); 
            ajaxFunction(sid,obj)//自加联动请求函数
        });
        this.visibleRows = this.getDomRows();
        this.selectedIndex = -1;
    };
    var ajaxFunction = function(sid,obj){  
            if(obj.types){ 
                //做联动效果  
                if (sid) {  
                    $.getJSON( obj.url,{sid:sid,grade:obj.grade,teacher:obj.teacher,department:obj.department,class:obj.class},function(mydata) { 
                        var option ='<option value="">全部</option>'; 
                        if(obj.teacher=="1"){
                            if(mydata&&mydata.teachers){
                                option += eachFunction(mydata.teachers);
                            }
                        }else if(obj.grade=="1"){
                            if(mydata&&mydata.grades){
                                option += eachFunction(mydata.grades);
                            } 
                        }else if(obj.department=="1"){
                            if(mydata&&mydata.departments){
                                option += eachFunction(mydata.departments);
                            } 
                        }else if(obj.class=="1"){
                            if(mydata&&mydata.classs){
                                option += eachFunctionClass(mydata.classs); 
                            } 
                        }    
                        $("#"+obj.selectid).html(option);
                    }); 
                   
                }  
            }
    };
    var eachFunction = function(data){
        var option ='';  
        if(data){
    ...
    var eachFunctionClass = function(data){
        var option =''; 
        if(data){
            $.each(data,function(i,v){
    ...
    Class.prototype.getListHtml = function(data) {
        ...
        for(var i=0; i < data.length; i++) {
            ...
            html += '<tr class="stc-lrow ' + oddRow + '" data-stc-id="' + d.id + '" data-stc-sid="'+d.vid+'">'
                  + '<td>'
    ...
    Class.prototype.getHtml = function(containerId) {
        return '<span class="stc-box" id="' + containerId + '-ddb">'
             ...
             + '<tr>'
             + ' <td><input type="text" style="width:190px " placeholder="选择学校" sname="" sid="" class="stc-input" id="' + containerId + '-ddi" autocomplete="off"/></td>'
