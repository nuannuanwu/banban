...
var STComboBox = (function($) {
    ...
    Class.prototype.onKeyDown = function(evt) { 
        if(evt.which == keycodes.enter) {
            ...
            this.$('ddi').val(this.data[selectedIndex].obj.name);
            this.hideList();
            return false;
        }
    ...
    Class.prototype.getFilteredRows = function(value) { 
        ...
        //console.log(this.filterCache+"--111-");
        var showRows = {};
        var match = null;
        this.visibleRows = [];
        var rows = this.getDomRows();
    ...
    Class.prototype.populateList = function(data) {
        ...
        for(var i=0; i < data.length; i++) {
            //alert(data[i].name);
            this.filterCache += i + '`' + data[i].obj.name+data[i].obj.pinYin+ '#';
        ...
        $list.find('.stc-lrow').hover(function() {
            ...
            self.getInput().val(self.data[selectedIndex].obj.name);
        });
       ...
       // console.log(data)
        for(var i=0; i < data.length; i++) {
            ...
            html += '<tr class="stc-lrow ' + oddRow + '" data-stc-id="' + d.id + '"'
                  ...
                  + '<td>'
                  + '<span class="stc-text" id="' + containerId + '-ddtext' + i + '">' +d.obj.name + '</span>'
                  + '</td>'
                  + '</tr>';
        }
        html += '</table>';
    ...
    Class.prototype.getHtml = function(containerId) {
        return '<span class="stc-box" id="' + containerId + '-ddb">'
             ...
             + '<tr>'
             + ' <td><input type="text" maxlength="20" class="stc-input" id="' + containerId + '-ddi" autocomplete="off"/></td>'
