       ...
       function  drawImgInit ( drawFillter ,drawWaterMark ) {
        ...
        function touchDragTransform(ev) {
          ...
          switch(ev.type) {
            ...
            // rotation = last_rotation + ev.gesture.rotation;
		      rotation = 0;
              scale = Math.max(0.2, Math.min(last_scale * ev.gesture.scale, 2));
            //scale = Math.max(0.3, Math.min(last_scale * ev.gesture.scale,2));
        ...
        function redrawImage(xpos,ypos) {
			 ...
			 // alert(options.image.height)
			  ...
			  context.restore();
			  if (drawFillter) {
			  context.drawImage (fillterImg,0,0,fillterImg.width,fillterImg.height,options.fillterImage.posx,options.fillterImage.posy,fillterImg.width*0.8,fillterImg.height*0.8);}
        }
    };
	};
})(Zepto)
